...
int main(int argc, char *argv[]){
	...
	enemies e1 = generate_enemies(1000,100,ev);
	ev.push_back(e1);
	enemies e2 = generate_enemies(1000,300,ev);
	ev.push_back(e2);
	enemies e3 = generate_enemies(1000,500,ev);
	ev.push_back(e3);
	enemies e4 = generate_enemies(1000,700,ev);
	ev.push_back(e4);
	...
	while(w.isOpen()) {
		...
		c1.update(objects, c1.obtenerpos());
		e1.update(c1.obtenerpos());
		
		
		
		for(enemies &e : ev) e.update(c1.obtenerpos());
		//Generar balas
